...
void swapContextOnTop(Context* pCurrent, Context* pToSwitchTo, std::function<void(Context* parentContext)> onTop) {
    using OnTopFunc = std::function<void(Context* parentContext)>;
    auto* copy = new OnTopFunc(std::move(onTop));

    // TODO: support for exceptions
    swap_context_on_top(pCurrent, pToSwitchTo, copy, [](Context* parentContext, void* pUserData) {
